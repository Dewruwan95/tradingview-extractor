...
def process_all_companies(
    ...
    try:
        ...
        total_companies = min(len(company_codes), max_companies) if max_companies else len(company_codes)
        print(f"\nProcessing {total_companies} companies with {rate_limit}s delay...")
        
        processed = 0
        progress_bar = tqdm(company_codes[:total_companies], desc="Processing", unit="company")
